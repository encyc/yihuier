...
class PipelineModule:
    ...
    def product_test(self) -> pd.DataFrame:
        ...
        # 日期变量转成二分类变量
        self.yihuier_instance.data = self.yihuier_instance.dp_module.date_var_shift_binary(
            self.yihuier_instance.get_date_variables(), replace=True
        )
        # 填充类别型变量空值
        self.yihuier_instance.dp_module.fillna_cate_var(
            self.yihuier_instance.get_categorical_variables(), "class", "unkonwn"
        )
        # 打印类别型变量的违约率
        ...
        # 填充数值型变量空值
        self.yihuier_instance.data = self.yihuier_instance.dp_module.fillna_num_var(
            self.yihuier_instance.get_numeric_variables(),
            fill_type="class",
            fill_class_num=MISSING_VALUE_NEG_999,
        )
        # 删除数值型变量
        self.yihuier_instance.data = self.yihuier_instance.dp_module.delete_missing_var(
            threshold=0.01
        )
        # 展示空值分布
        ...
        xg_fea_imp, _, _ = self.yihuier_instance.var_select_module.select_xgboost(
            self.yihuier_instance.get_numeric_variables()
        )
        rf_fea_imp, _ = self.yihuier_instance.var_select_module.select_rf(
            self.yihuier_instance.get_numeric_variables()
        )
