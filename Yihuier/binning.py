...
class BinningModule:
    ...
    def binning_cate(self, col_list: list[str]) -> tuple[list, list, list]:
        ...
        for col in col_list:
            ...
            for i in d2.odds:
                if i >= all_odds:
                    GB_index = str(round((i / all_odds) * 100, 0)) + "G"
                else:
                    GB_index = str(round((all_odds / i) * 100, 0)) + "B"
    ...
    def __split_data(self, df, col, split_num):
        ...
        values = sorted(df2[col])  # 对变量的值从小到大进行排序
        split_value = [values[i] for i in split_index]  # 分割点对应的value
        split_value = sorted(set(split_value))  # 分割点的value去重排序
        return split_value
    ...
    def __assign_group(self, x, split_bin):
        ...
        if x <= min(split_bin):
            ...
            for i in range(n - 1):
                if (
                    split_bin[i] < x <= split_bin[i + 1]
                ):  # 如果x在两个分割点之间，则x映射为分割点较大的值
    ...
    def __bin_bad_rate(self, df, col, target, grantRateIndicator=0):
        ...
        dict_bad = dict(zip(regroup[col], regroup["bad_rate"], strict=True))  # 转为字典形式
    ...
    def __cal_chi2(self, df, all_bad_rate):
        ...
        combined = zip(
            df2["expected"], df2["bad"], strict=True
        )  # 遍历每组的坏用户期望数量和实际数量
        chi = [(i[0] - i[1]) ** 2 / i[0] for i in combined]  # 计算每组的卡方值
        chi2 = sum(chi)  # 计算总的卡方值
        return chi2
    ...
    def __assign_bin(self, x, cutoffpoints):
        ...
        bin_num = len(cutoffpoints) + 1  # 箱体个数
        if (
            not cutoffpoints or x <= cutoffpoints[0]
        ):  # 如果cutoffpoints为空或x小于最小的cutoff点，则映射为Bin 0
            return "Bin 0"
        elif x > cutoffpoints[-1]:  # 如果x大于最大的cutoff点，则映射为Bin(bin_num-1)
            return f"Bin {bin_num - 1}"
        else:
            for i in range(0, bin_num - 1):
                if (
                    cutoffpoints[i] < x <= cutoffpoints[i + 1]
                ):  # 如果x在两个cutoff点之间，则x映射为Bin(i+1)
                    return f"Bin {i + 1}"

    def __chi_merge(self, df, col, target, max_bin=5, min_binpct=0):
        col_unique = sorted(set(df[col]))  # 变量的唯一值并排序
        n = len(col_unique)  # 变量唯一值得个数
        df2 = df.copy()
        if (
            n > 100
        ):  # 如果变量的唯一值数目超过100，则将通过__split_data和__assign_group将x映射为split对应的value
            split_col = self.__split_data(
                df2, col, 100
            )  # 通过这个目的将变量的唯一值数目人为设定为100
        ...
        (dict_bad, regroup, all_bad_rate) = self.__bin_bad_rate(
            df2, "col_map", target, grantRateIndicator=1
        )
        col_map_unique = sorted(set(df2["col_map"]))  # 对变量映射后的value进行去重排序
        group_interval = [
            [i] for i in col_map_unique
        ]  # 对col_map_unique中每个值创建list并存储在group_interval中

        while len(group_interval) > max_bin:  # 当group_interval的长度大于max_bin时，执行while循环
            chi_list = []
            for i in range(len(group_interval) - 1):
                temp_group = (
                    group_interval[i] + group_interval[i + 1]
                )  # temp_group 为生成的区间,list形式，例如[1,3]
            ...
            # 将卡方值最小的一对区间进行合并
            group_interval[best_combined] = (
                group_interval[best_combined] + group_interval[best_combined + 1]
            )
            # 删除合并前的右区间
        ...
        # 检查是否有箱只有好样本或者只有坏样本
        df2["col_map_bin"] = df2["col_map"].apply(
            lambda x: self.__assign_bin(x, cutoffpoints)
        )  # 将col_map映射为对应的区间Bin
        # 计算每个区间的违约率
        ...
        while min_bad_rate == 0 or max_bad_rate == 1:
            bad01_index = regroup[
                regroup["bad_rate"].isin([0, 1])
            ].col_map_bin.tolist()  # 违约率为1或0的区间
    ...
    def iv_num(self, col_list: list[str], **kwargs) -> pd.DataFrame:
        ...
        method = kwargs.get("method", "freq")

        bin_df, iv_value = self.binning_num(
            col_list, max_bin=max_bin, min_binpct=min_binpct, method=method
        )
    ...
    def binning_num_manual(
        self, df: pd.DataFrame, target: str, col: str, cut: list
    ) -> tuple[list, list]:
        """
        df:数据集
        target:目标变量的字段名
    ...
    # 自定义分箱
    def binning_self(
        self, col: str, cut: list | None = None, right_border: bool = True
    ...
    def plot_woe(
        ...
        for i, df in zip(range(1, plt_num + 1, 1), bin_df, strict=True):
    ...
    def woe_monoton(self) -> tuple[list, pd.DataFrame]:
        ...
        for woe_df in bin_df:
            ...
            if woe_df.shape[0] == 2:
                ...
                woe_not_monoton = [
                    (woe_list[i] < woe_list[i + 1] and woe_list[i] < woe_list[i - 1])
                ...
                if True in woe_not_monoton:
                    ...
                    col_list.append(col_name)
                    woe_judge.append("True")
        woe_judge_df = pd.DataFrame({"col": col_list, "judge_monoton": woe_judge})
    ...
    def woe_large(self) -> tuple[list, pd.DataFrame]:
        ...
        for woe_df in bin_df:
            ...
            if len(woe_large) > 0:
                ...
                col_list.append(col_name)
                woe_judge.append("False")
        woe_judge_df = pd.DataFrame({"col": col_list, "judge_large": woe_judge})
    ...
    def woe_df_concat(self) -> pd.DataFrame:
        ...
        for df in bin_df:
            woe_df = (
                df.reset_index()
                .assign(col=df.columns.values[0])
                .rename(columns={df.iloc[:, 0].name: "bin"})
            )
